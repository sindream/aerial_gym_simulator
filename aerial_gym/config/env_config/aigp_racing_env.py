...
TRACK_INCLUDE_ASSET_TYPE = {
    f"aigp_race_gate_{gate_index + 1:02d}": True
    for gate_index in range(len(TRACK_GATE_ASSET_CLASSES))
}

TRACK_ASSET_TYPE_TO_DICT_MAP = {
    f"aigp_race_gate_{gate_index + 1:02d}": gate_asset_class
    for gate_index, gate_asset_class in enumerate(TRACK_GATE_ASSET_CLASSES)
}


class AIGPRacingEnvCfg:
    class env:
        num_envs = 128
        num_env_actions = 0
        env_spacing = ENV_SPACING

        # base_sim_1ms runs physics at 1000 Hz. Holding one action for 20
        # physics steps exposes the policy at 50 Hz.
